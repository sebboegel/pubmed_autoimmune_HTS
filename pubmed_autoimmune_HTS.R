...
#search performed Oct 26, 2020
pubmed_query_string="(NGS OR \"next generation sequencing\" OR RNA-Seq OR \"mRNA sequencing\" OR \"RNA sequencing\" OR \"RNA-sequencing\" OR \"transcriptome sequencing\" OR \"whole exome sequencing\" OR \"whole-exome sequencing\" OR \"high throughput sequencing\" OR \"high-throughput sequencing\" OR \"DNA sequencing\" OR \"RNA sequencing\" OR \"RNA-sequencing\" OR \"DNA-sequencing\" OR WXS OR WGS OR \"whole-genome sequencing\" OR \"whole genome sequencing\") AND (autoimmunity OR \"autoimmune disease\" OR \"autoimmune diseases\" ))"


pubmed_ids <- get_pubmed_ids(pubmed_query_string)
#1136 entries
abstracts_xml <- fetch_pubmed_data(pubmed_ids,retmax = 3000)
abstracts_list <- articles_to_list(abstracts_xml)
saveRDS(abstracts_list,file="/Users/sebastian/pubmed_autoimmune_HTS/abstracts_list_for_keywords.RDS")
final=c("")
...
#310 entries in finalTable_freq


#search performed Oct 26, 2020

pubmed_query_string="(NGS OR \"next generation sequencing\" OR RNA-Seq OR \"mRNA sequencing\" OR \"RNA sequencing\" OR \"RNA-sequencing\" OR \"transcriptome sequencing\" OR \"whole exome sequencing\" OR \"whole-exome sequencing\" OR \"high throughput sequencing\" OR \"high-throughput sequencing\" OR WXS OR WGS OR \"whole-genome sequencing\" OR \"whole genome sequencing\") AND (\"rheumatoid arthritis\" OR \"systemic lupus erythematosus\" OR \"multiple sclerosis\" OR \"type 1 diabetes\" OR \"psoriasis\" OR \"myasthenia gravis\" OR \"autoimmune liver disease\" OR \"autoimmune thyroiditis\" OR \"hashimoto thyroiditis\" OR \"inflammatory bowel disease\" OR \"Crohn's Disease\" OR \"Ulcerative colitis\")"
...
abstracts_xml <- fetch_pubmed_data(pubmed_ids,retmax = 3000)
abstracts_list <- articles_to_list(abstracts_xml)
diseases=c("rheumatoid arthritis","systemic lupus erythematosus","multiple sclerosis","type 1 diabetes","psoriasis","myasthenia gravis","autoimmune liver disease", "autoimmune thyroiditis"," hashimoto thyroiditis", "inflammatory bowel disease")
RNA=c("rna sequencing", "transcriptome sequencing", "rna-seq","rna-based next-generation sequencing","mrna profiles","rna-sequencing")
WXS=c("whole exome sequencing","whole-exome sequencing","whole-exome-sequencing","amplicon sequencing","exome sequencing")
WGS=c("whole-genome sequencing","whole genome sequencing","whole-genome shotgun sequencing","genome sequencing")
...
result_df[result_df$pmid=="17309832",]$assay="DNA gene panel" 
...
result_df[result_df$pmid=="21070896",]$assay="WES" #case report

result_df[result_df$pmid=="29766049",]$year="2018" 

result_df[result_df$pmid=="27822551",]$year="2016"
result_df[result_df$pmid=="27822551",]$assay="Metagenomics"
result_df[result_df$pmid=="27772553",]$year="2016"
result_df[result_df$pmid=="31798832",]$assay="DNA gene panel"
result_df[result_df$pmid=="31798832",]$year=2019
result_df[result_df$pmid=="31223483",]$year=2019
result_df[result_df$pmid=="21951576",]$year=2011
...
deletepmids=c("32264838","31494232","31744152","30632919","22933063","25373782","25541309","25799352","26182405","26705675","28012117","28286571","28417081",
              ...
              "24223582","24013639","20514598","21262369","22046267","17898275","33051416","30922683")
...
result_df[result_df$assay=="WGS,WGS",]$assay="WGS"
result_df[result_df$assay=="WGS,WGS,WGS",]$assay="WGS"
result_df[result_df$assay=="Epigenomics,Epigenomics",]$assay="Epigenomics"
result_df[result_df$assay=="Metagenomics,Metagenomics",]$assay="Metagenomics"

result_df[result_df$assay=="Metagenomics,WES,WGS,WGS",]$assay="Metagenomics,WES,WGS"
result_df[result_df$assay=="Metagenomics,WGS,WGS",]$assay="Metagenomics,WGS"
result_df[result_df$assay=="Metagenomics,WGS,WGS,WGS",]$assay="Metagenomics,WGS"

result_df[result_df$assay=="Epigenomics,WGS,WGS",]$assay="Epigenomics,WGS"
result_df[result_df$assay=="Epigenomics,Epigenomics,RNA-Seq",]$assay="Epigenomics,RNA-Seq"
result_df[result_df$assay=="Epigenomics,Epigenomics,RNA-Seq,single",]$assay="Epigenomics,RNA-Seq,scRNA-Seq"
result_df[result_df$assay=="WES,WGS,WGS",]$assay="WGS,WES"
...
result_df[result_df$assay=="Metagenomics,WES",]$assay_main="Other"
result_df[result_df$assay=="Metagenomics,WGS",]$assay_main="Other"
result_df[result_df$assay=="Metagenomics,WES,WGS",]$assay_main="Other"
result_df[result_df$assay=="Epigenomics",]$assay_main="Other"
result_df[result_df$assay=="Epigenomics,WGS",]$assay_main="Other"

result_df[result_df$assay=="Epigenomics,RNA-Seq,scRNA-Seq",]$assay_main="Other"
...
ggplot(data=result_df, aes(x=fct_infreq(disease),fill=assay_main)) + geom_bar(position="stack",stat="count") + 
  ...
  geom_text(data = df2, aes(x=disease,y=330,label=number,angle=60),inherit.aes = FALSE) +
  ...
  scale_fill_brewer(palette = "Paired") + scale_y_continuous(breaks=seq(0,330,10)) +
...
ggplot(data=subset(result_df_unique, result_df_unique$assay %in% c("scRNA-Seq","WES,scRNA-Seq","Epigenomics,RNA-Seq,scRNA-Seq")), aes(x=year,fill=assay)) + geom_bar(position="stack",stat="count") + 
...
abundance_plot=ggplot(data=result_df_unique, aes(x=year,fill=assay_main)) + geom_bar(position="stack",stat="count") + 
  labs(y="# unique publications on pubmed") +
  geom_text(data = df2, aes(x=year,y=300,label=number,angle=60),inherit.aes = FALSE) +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, size=13, colour="black"), 
        axis.title.x = element_blank(), axis.text.y=element_text(colour="black", size = 13), axis.title.y=element_text(colour="black", size = 14)) +
  theme(legend.title = element_blank()) +
